...
# Clave de YouTube Data API v3 — OBLIGATORIO configurarla como variable de entorno.
# Nunca debe estar hardcodeada (la clave anterior quedó expuesta en git y debe rotarse).
YOUTUBE_API_KEY = os.getenv("YOUTUBE_API_KEY", "")
# Orígenes permitidos para CORS: "*" (cualquiera) o lista separada por comas.
ALLOWED_ORIGINS = os.getenv("ALLOWED_ORIGINS", "*")
MEDIA_DIR = "media"
DIST_DIR = "frontend/dist"  # build del front nuevo (Vue); si no existe, se sirve el monolito index.html
DB_FILE = "db.json"
...
class Room:
    def __init__(self, room_id: str, name: str = "", created_by: Optional[str] = None,
        ...
        self.members: Dict[WebSocket, str] = {}  # ws -> user_id (presencia en la sala)
...
async def room_broadcast_presence(room: Room):
    """Difunde quién está en la sala: conteo, lista de user_ids y host."""
    members = [room.members[w] for w in list(room.connections) if w in room.members]
    await room.broadcast({"type": "presence", "data": {
        "count": len(members),
        "listeners": members,
...
async def lifespan(app: FastAPI):
    # Arranque: lanzar tareas de fondo (reemplaza el deprecado @app.on_event("startup"))
    ticker_task = asyncio.create_task(ticker())
    cleanup_task = asyncio.create_task(cleanup_old_sessions())
    try:
        yield
    finally:
        # Apagado: cancelar tareas de fondo limpiamente
        ticker_task.cancel()
        cleanup_task.cancel()

app = FastAPI(title="Music Realtime FastAPI", lifespan=lifespan)

# Configurar CORS. Nota: allow_credentials NO puede ser True junto con allow_origins="*"
# (los navegadores lo rechazan). La app no usa cookies, así que va en False.
_cors_origins = ["*"] if ALLOWED_ORIGINS.strip() == "*" else [o.strip() for o in ALLOWED_ORIGINS.split(",") if o.strip()]
app.add_middleware(
    CORSMiddleware,
...
async def get_video_duration_api(video_id: str) -> int:
    ...
    try:
        ...
        # API key desde variable de entorno (ver YOUTUBE_API_KEY arriba)
        api_key = YOUTUBE_API_KEY
        if not api_key:
            print("[YT-API] YOUTUBE_API_KEY no configurada; se omite la consulta de duración por API")
            return None
...
# ---- Audio real vía yt-dlp (cliente android) para reproducción con <audio> ----
# Habilita background fiable (HTMLAudioElement) y espectro real (Web Audio), a cambio
# de extraer el stream en el servidor (sujeto a límites de YouTube).
...
def _extract_audio_ytdlp(video_id: str) -> Dict[str, Any]:
    ...
    url = fmt.get("url")
    if not url:
        raise Exception("yt-dlp no devolvió URL de audio")
    return {
        "stream_url": url,
        "headers": fmt.get("http_headers") or {},
        "content_type": f"audio/{fmt.get('ext', 'mp4')}",
        "timestamp": time.time(),
    }
...
@app.api_route("/audio/{video_id}", methods=["GET", "HEAD"])
async def audio_proxy(request: Request, video_id: str):
    """Proxy de audio real (mismo origen) para un id de YouTube: extrae con yt-dlp y
    reenvía los bytes con los headers correctos + soporte de Range. Lo usa el
    HtmlAudioEngine del frontend (background fiable + espectro real)."""
    import httpx
    try:
        src = await _get_audio_source(video_id)
    except Exception as e:
        return JSONResponse({"error": f"No se pudo extraer el audio: {e}"}, status_code=502)

    fwd = dict(src["headers"])  # incluye el User-Agent del cliente android
    ...
    try:
        req = client.build_request("GET", src["stream_url"], headers=fwd)
        upstream = await client.send(req, stream=True)
        upstream.raise_for_status()
    except Exception as e:
        await client.aclose()
        # La URL pudo expirar/fallar: invalidar cache para reintentar limpio
        async with _audio_cache_lock:
    ...
    ctype = upstream.headers.get("content-type", src.get("content_type", "audio/mp4"))
    ...
    return StreamingResponse(body(), status_code=upstream.status_code, media_type=ctype, headers=out_headers)
...
async def stream(request: Request, track_id: str):
    ...
    if mode == "direct":
        ...
        # Proxy con streaming real (sin bufferizar todo el audio en memoria) y soporte de Range.
        import httpx
        ...
        try:
            ...
            # Abrir cliente y request en modo streaming. NO usar 'async with' aquí: cerraría
            # el cliente antes de que StreamingResponse termine de emitir los chunks.
            client = httpx.AsyncClient(timeout=timeout, follow_redirects=True)
            try:
                req = client.build_request("GET", stream_url, headers=headers_to_youtube)
                youtube_response = await client.send(req, stream=True)
            ...
            # Generador que transmite los chunks a medida que llegan y cierra recursos al final.
            async def stream_youtube_content():
                try:
                    async for chunk in youtube_response.aiter_bytes(chunk_size=65536):
                        yield chunk
                finally:
                    await youtube_response.aclose()
                    await client.aclose()
...
async def api_list_rooms():
    """Lista todas las rooms disponibles (sin exponer el password_hash)"""
    all_rooms = rooms_read()
    rooms_safe = [
        {
            "id": r.get("id"),
            ...
            "requires_password": (not r.get("is_public", True)) and bool(r.get("password_hash")),
            "queue": r.get("queue", []),
        }
        for r in all_rooms.values()
    ]
    return {"rooms": rooms_safe}
...
async def api_get_room(room_id: str):
    ...
    # No crear rooms fantasma desde un simple GET: solo devolver las existentes (o la default).
    if room_id != "default" and room_id not in rooms_read():
        return JSONResponse({"error": "Room no encontrada"}, status_code=404)
...
def index():
    ...
    return FileResponse(idx if os.path.isfile(idx) else "index.html")
...
async def ws_room_endpoint(ws: WebSocket, room_id: str):
    ...
    room.members[ws] = ws.query_params.get("user_id") or f"anon-{id(ws)}"
    await room_broadcast_presence(room)
            ...
            if t == "queue:add":
                ...
                try:
                    if hybrid_mode:
                        ...
                        # Read-modify-write protegido con _db_lock para no corromper db.json.
                        async with _db_lock:
                            db = db_read()
                            if url_or_id in db:
                                rec = db[url_or_id]
                                # Enriquecer un registro placeholder si ahora llega metadata real del cliente
                                meta_title = msg.get("title")
                                if meta_title and str(rec.get("title", "")).startswith("YouTube Video"):
                                    rec["title"] = meta_title
                                    if msg.get("artist"): rec["uploader"] = msg.get("artist")
                                    if msg.get("duration"): rec["seconds"] = rec["duration"] = int(msg.get("duration"))
                                    if msg.get("thumbnail"): rec["thumbnail"] = msg.get("thumbnail")
                                    db[url_or_id] = rec
                                    db_write(db)
                                    print(f"[HYBRID] Track {url_or_id} enriquecido: {rec['title']}")
                            else:
                                # Modo híbrido: usar la metadata que el cliente ya conoce (p. ej. desde
                                # la búsqueda); fallback a placeholder solo si no llega nada.
                                title = msg.get("title") or f"YouTube Video {url_or_id}"
                                duration = int(msg.get("duration")) if msg.get("duration") else 180
                                print(f"[HYBRID] Agregando track {url_or_id} en modo híbrido: {title}")
                                rec = {
                                    "id": url_or_id,
                                    "title": title,
                                    "seconds": duration,
                                    ...
                                    "mode": "hybrid",
                                    "timestamp": time.time(),
                                    "thumbnail": msg.get("thumbnail") or f"https://i.ytimg.com/vi/{url_or_id}/hqdefault.jpg",
                                    "uploader": msg.get("artist") or "YouTube",
                                }
...
# -------- SPA: servir el build del front (Vue) con fallback de rutas ----------
# IMPORTANTE: va DESPUÉS de todas las rutas /api, /audio, /stream, /health, /ws...
# para no eclipsarlas. Sirve los assets del build y devuelve index.html para las
# rutas del SPA (/search, /room/x, /playlist/x) cuando se recargan.
_SPA_CT = {
    ".js": "text/javascript",
    ".css": "text/css",
    ".svg": "image/svg+xml",
...
async def spa(full_path: str):
    base = os.path.realpath(DIST_DIR)
    target = os.path.realpath(os.path.join(DIST_DIR, full_path))
    # Archivo real del build (assets, favicon, manifest, sw...), con anti path-traversal
    if full_path and target.startswith(base + os.sep) and os.path.isfile(target):
        ext = os.path.splitext(target)[1].lower()
        return FileResponse(target, media_type=_SPA_CT.get(ext))
    # Resto de rutas -> index.html del SPA (o el monolito viejo si aún no hay build)
    idx = os.path.join(DIST_DIR, "index.html")
    return FileResponse(idx if os.path.isfile(idx) else "index.html")


# -------- Arranque --------
# Permite levantar la app con `python main.py`. En producción usar:
...
if __name__ == "__main__":
    ...
    print(f"[STARTUP] Iniciando servidor en http://{host}:{port} (reload={reload})")
    # Con reload se requiere el import string; sin reload pasamos el objeto app
    # directamente para no reimportar el módulo (evita duplicar los logs de arranque).
    uvicorn.run("main:app" if reload else app, host=host, port=port, reload=reload)
